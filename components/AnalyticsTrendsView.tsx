      ...
      <div className="rounded-lg border border-[#23304a] bg-[#131b2e] p-4 sm:p-5 shadow-sm">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#0d1524] border border-[#23304a] text-sky-400">
          ...
          <div className="flex items-center gap-1.5 bg-[#0d1524] p-1 rounded-lg border border-[#23304a] font-mono text-xs">
            <button
              onClick={() => setActiveMetric('trays')}
              className={`px-3 py-1.5 rounded-md font-medium transition-all cursor-pointer ${
                activeMetric === 'trays' ? 'bg-sky-600 text-white shadow-xs font-semibold' : 'text-slate-400 hover:text-slate-200 hover:bg-[#162238]'
              }`}
            >
              Tray Temps (1, 4, 7)
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-medium transition-all cursor-pointer ${
                activeMetric === 'vacuum' ? 'bg-sky-600 text-white shadow-xs font-semibold' : 'text-slate-400 hover:text-slate-200 hover:bg-[#162238]'
              }`}
            >
              Deodorizer Vacuum
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-medium transition-all cursor-pointer ${
                activeMetric === 'steam' ? 'bg-sky-600 text-white shadow-xs font-semibold' : 'text-slate-400 hover:text-slate-200 hover:bg-[#162238]'
              }`}
            >
              Steam Pressures
            </button>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {activeMetric === 'trays' ? (
              <LineChart data={timeSeriesData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: '#131b2e', borderColor: '#23304a', borderRadius: '0.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.5)', color: '#f1f5f9' }}
                  itemStyle={{ fontSize: 12, fontFamily: 'monospace', color: '#f1f5f9' }}
                />
                <Legend wrapperStyle={{ fontSize: 12, paddingTop: 8 }} />
                <Line type="monotone" dataKey="tray1" name="Tray 1 (°C)" stroke="#38bdf8" strokeWidth={2} dot={{ r: 3 }} />
              ...
              <LineChart data={timeSeriesData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                />
                <Legend wrapperStyle={{ fontSize: 12, paddingTop: 8 }} />
                <Line type="monotone" dataKey="boosterPress" name="Booster Pressure (Bar)" stroke="#38bdf8" strokeWidth={2} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="ejectorPress" name="Ejector Pressure (Bar)" stroke="#0ea5e9" strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            )}
          </ResponsiveContainer>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={paretoData} margin={{ top: 10, right: 10, bottom: 25, left: -10 }}>
                ...
                <Tooltip
                  ...
                  itemStyle={{ fontSize: 12, fontFamily: 'monospace', color: '#f1f5f9' }}
                />
                <Bar dataKey="count" name="Rejection Count" fill="#f43f5e" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>

          <div className="mt-3 text-xs text-slate-300 bg-[#0d1524] p-3 rounded-lg border border-[#23304a] font-mono">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={productRejections} margin={{ top: 10, right: 10, bottom: 20, left: -10 }}>
                ...
                <Tooltip
                  ...
                  itemStyle={{ fontSize: 12, fontFamily: 'monospace', color: '#f1f5f9' }}
                />
                <Bar dataKey="lots" name="Total Lots Produced" fill="#334155" radius={[3, 3, 0, 0]} />
                <Bar dataKey="rejects" name="Rejected Lots" fill="#f43f5e" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>

          <div className="mt-3 text-xs text-slate-300 bg-[#0d1524] p-3 rounded-lg border border-[#23304a] font-mono">
            <strong>Corrective Target:</strong> Highest attention needed during grade switchovers to <span className="text-sky-400 font-semibold">PL 65 Matsuyama</span>.
          </div>
        </div>
      </div>
    </div>
